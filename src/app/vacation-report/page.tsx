...
export default function VacationReportPage() {
  ...
  const selectedLeaveEntries = filteredLeaves.filter(leave => selectedLeaves.includes(leave.id))
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Type de congés */}
          <div>
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Type de congés</h3>
            <div className="flex flex-wrap gap-2">
              {(['rtt', 'cp', 'cet'] as const).map(type => (
                ...
                >
                  ...
                  <span className="text-sm font-medium">{getLeaveTypeLabel(type)}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Période */}
          <div>
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Période</h3>
            <div className="flex flex-wrap gap-2">
              {[
                ...
                filteredLeaves.map((leave) => (
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <span className="text-lg">{getLeaveTypeIcon(leave.type)}</span>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <div className="p-6">
            <div className="space-y-4 text-sm">
              <div>
                <span className="font-medium text-gray-700 dark:text-gray-300">À:</span>
                <span className="ml-2 text-gray-900 dark:text-white">dlepetit.maa@gmail.com</span>
              </div>
              <div>
                <span className="font-medium text-gray-700 dark:text-gray-300">Objet:</span>
                <span className="ml-2 text-gray-900 dark:text-white">Rapport de congés {currentYear}</span>
              </div>
              <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                <p className="text-gray-900 dark:text-white mb-4">Hi Carlo,</p>
                <p className="text-gray-900 dark:text-white mb-4">Please find my vacation report below:</p>
                ...
                {selectedLeaveEntries.length > 0 ? (
                        ...
                        <div key={type}>
                          ...
                          <ul className="ml-4 space-y-1">
                            {typeLeaves.map(leave => (
                              <li key={leave.id} className="text-gray-700 dark:text-gray-300">
                                • {format(new Date(leave.startDate), 'dd MMM yyyy', { locale: fr })}
                                {leave.startDate !== leave.endDate && (
                                  <span>, {format(new Date(leave.endDate), 'dd MMM yyyy', { locale: fr })}</span>
                                )}
                              </li>
                            ))}
                          </ul>
                  ...
                  <p className="text-gray-500 dark:text-gray-400 italic">Aucun congé sélectionné</p>
                )}
                
                <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                  <p className="text-gray-900 dark:text-white">Regards,</p>
                  <p className="text-gray-900 dark:text-white">Daniel</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {selectedLeaveEntries.length > 0 && (
        <div className="mt-6 bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white bg-orange-100 dark:bg-orange-900/20 px-3 py-1 rounded mb-4">
            Résumé
          </h3>
            ...
            {['rtt', 'cp', 'cet'].map(type => {
              ...
              return (
                <div key={type} className="text-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">{getLeaveTypeLabel(type)}</div>
                  <div className="text-xl font-bold text-gray-900 dark:text-white">{totalDays}</div>
                </div>
              )
            })}
          </div>
